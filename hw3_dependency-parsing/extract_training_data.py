...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        if len(state.stack) < 3:
            stack_rep = copy.deepcopy(state.stack)
            while len(stack_rep) < 3:
                stack_rep.append("<NULL>")
        else:
            stack_rep = state.stack[-3:]
        
        if len(state.buffer) < 3:
            buff_rep = copy.deepcopy(state.buffer)
            while len(buff_rep) < 3:
                buff_rep.append("<NULL>")
        else:
            buff_rep = state.buffer[-3:]     
        
        feature = stack_rep + buff_rep # concatenate

        for i, idx in enumerate(feature): # convert indicies to words or special tokens   

            if idx == "<NULL>":
                continue
            elif pos[int(idx)] in ["<CD>","<NNP>"]: # check for special cases
                feature[i] = pos[int(idx)]
            elif idx == 0:
                feature[i] = "<ROOT>"
            elif words[int(idx)].lower() not in self.word_vocab: # check if unknown
                feature[i] = "<UNK>"
            else:
                feature[i] = words[feature[i]].lower() # minimal processing to improve performance
         
        feature = [self.word_vocab[i] for i in feature] # map words to vocab index mappings
